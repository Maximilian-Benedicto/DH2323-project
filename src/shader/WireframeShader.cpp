...
void WireframeShader::drawLine(Uint32* pixelBuffer, int width, int height, int x0, int y0, int x1,
                               int y1, Uint32 color) {
...
void WireframeShader::render(Uint32* pixelBuffer, int width, int height, const Model& model,
    ...
    // Compute the camera right and up vectors
    ...
    for (int idx = 0; idx < nodeCount; ++idx) {
        ...
        // If BVH visualization is enabled, draw the bounding box of this node in wireframe.
        if (!node.isLeaf() && isShowingBvh) {
            ...
            if (min.x > max.x || min.y > max.y || min.z > max.z)
                continue;
            if (isinf(min.x) || isinf(min.y) || isinf(min.z) || isinf(max.x) || isinf(max.y) ||
                isinf(max.z))
                continue;
            ...
            // Compute the color of the bounding box based on its size
            float boxSize = length(max - min);
            Uint32 alpha = std::clamp((int)(boxSize * 100.0f), 20, 255);
            Uint32 lightColor = (alpha << 24) | 0x000000FF;

            // Draw the edges of the bounding box if they are within view
            for (int i = 0; i < 12; ++i) {
                ...
                if (p1Cam.z < -0.1f && p2Cam.z < -0.1f) {
                    ...
                    drawLine(pixelBuffer, width, height, (int)px1, (int)py1, (int)px2, (int)py2,
                             lightColor);
                }
            }
        } else if (!isShowingBvh) {  // Draw the triangles within the leaf node
            ...
            for (size_t i = start; i < end; i++) {
                ...
                if (!outOfBounds[0] && !outOfBounds[1])
                    drawLine(pixelBuffer, width, height, p[0][0], p[0][1], p[1][0], p[1][1],
                             0xFF00FF00);
                if (!outOfBounds[1] && !outOfBounds[2])
                    drawLine(pixelBuffer, width, height, p[1][0], p[1][1], p[2][0], p[2][1],
                             0xFF00FF00);
                if (!outOfBounds[2] && !outOfBounds[0])
                    drawLine(pixelBuffer, width, height, p[2][0], p[2][1], p[0][0], p[0][1],
                             0xFF00FF00);
            }
        }
    }
