...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Combine 4 Diana images into a 1x4 grid."
    )
    parser.add_argument(
        "folder",
        nargs="?",
        default=".",
        help="Folder containing Diana*.png files (default: current folder).",
    )
    parser.add_argument(
        "--output",
        default="DianaGrid.png",
        help="Output image name (default: DianaGrid.png).",
    )
    ...
    if len(images) != 4:
        raise SystemExit("Expected 4 images in the order list.")
